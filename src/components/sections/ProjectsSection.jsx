...
export default function ProjectsSection() {
  ...
  const onMouseMove = useCallback(e => {
    ...
    liveOff.current = Math.max(-40, Math.min(
      startOff.current - (e.clientX - startX.current), max + 40
    ));
    if (trackRef.current)
      trackRef.current.style.transform = `translateX(-${liveOff.current}px)`;
  }, [projects.length, cardW]);
  ...
  const onTouchMove = e => {
    const max = (projects.length - 1) * cardW();
    liveOff.current = Math.max(-40, Math.min(
      startOff.current - (e.touches[0].clientX - startX.current), max + 40
    ));
    if (trackRef.current)
      trackRef.current.style.transform = `translateX(-${liveOff.current}px)`;
  };
  ...
  // Helper: color del status
  const getStatusColor = (s) => {
    if (!s) return '#007bff';
    const lc = s.toLowerCase();
    if (lc.includes('producc') || lc.includes('complet')) return '#28ca41';
    if (lc.includes('desarroll') || lc.includes('progres')) return '#febc2e';
    if (lc.includes('pause') || lc.includes('pausa'))      return '#ff6b5e';
    return '#007bff';
  };
                ...
                return (
                  ...
                  >
                    <div className="pj-inner">
                      ...
                      {/* ── BANNER ── imagen o gradiente + icono ── */}
                      <div className="pj-banner">

                        {/* Fondo: imagen si existe */}
                        {p.imageUrl
                          ? <img src={p.imageUrl} alt={p.title} className="pj-img" />
                          : <div className="pj-banner-bg" style={{
                              background: `linear-gradient(135deg, ${pcolor}18 0%, #050912 100%)`
                            }} />
                        }

                        {/* Radar giratorio sutil */}
                        <div className="pj-radar" />

                        {/* Icono central — emoji o 💻 por defecto */}
                        {!p.imageUrl && (
                          <div className="pj-center-icon">
                        ...
                        {(p.category || p.type) && (
                          <div className="pj-type">{p.category || p.type}</div>
                        )}

                        {/* Badge status bottom-left */}
                        {p.status && (
                          <div className="pj-status">
                            <span className="pj-status-dot" style={{ background: sc }} />
                            <span style={{ color: sc }}>{p.status}</span>
                          </div>
                        )}
                      </div>
                      ...
                      <div className="pj-body">
                        ...
                        {/* Título */}
                        <div className="pj-name">{p.title || p.name || p.nombre}</div>

                        {/* Subtítulo */}
                        {p.subtitle && (
                          <div className="pj-subtitle">{p.subtitle}</div>
                        )}
                        ...
                        {tags.length > 0 && (
                          <div className="pj-tags">
                            ...
                            {tags.length > 4 && (
                              <span className="pj-tag">+{tags.length - 4}</span>
                            )}
                          </div>
                        )}
